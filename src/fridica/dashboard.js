...
async function controlTask(action,task,cleanupRevision) {
  try {
    const result=await api('/api/thread-action',{channel:task.channel,thread:task.thread,action,revision:task.control_revision,cleanup_revision:cleanupRevision});
    selected=null;detail=null;$('detail').hidden=true;
    notify({resume:result&&result.replayed?'Resumed. The latest unanswered message in this thread will be answered now.':result&&result.note?'Resumed for future messages. '+result.note:'Resumed for future messages. Earlier messages will not be replayed.',close:'Request closed locally. Automatic replies are stopped.',archive:'Request archived. Restore it from Requests → Archived.',restore:'Request restored.',clean:'Local request contents cleared. Project files and Slack messages are unchanged.'}[action]);
