...
A ``[parent] repos`` path in ``config.toml`` overrides the shared list for local testing.
"""
...
def default_repos_text() -> str:
    return files("fridica.parent").joinpath("repos.toml").read_text(encoding="utf-8")
