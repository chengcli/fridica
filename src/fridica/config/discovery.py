...
async def discover_from_config(path: Path) -> tuple[str, str, list[dict], list[str]]:
    ...
    slack = values.get("slack", {}) if isinstance(values.get("slack"), dict) else {}
    variable = slack.get("user_token_env", "SLACK_USER_TOKEN")
    if not isinstance(variable, str) or not re.fullmatch(r"[A-Za-z_][A-Za-z0-9_]*", variable):
        raise ValueError("slack.user_token_env must name an environment variable")
