...
function renderDetail() {
  ...
  if (unlocked && task.control_state) {
    ...
    if(task.control_state==='closed' || task.control_state==='active' && task.status==='complete') controls.append(button('Archive',()=>controlTask('archive',task)));
...
function renderSettings() {
  ...
  $('agent-config').replaceChildren();
  $('diagnostics').textContent=JSON.stringify({owner:state.config.owner,workspace:state.config.workspace,channels:state.config.channels,backend:state.config.backend,listener:state.health},null,2);
}
